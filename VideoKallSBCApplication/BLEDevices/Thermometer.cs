  ...
  public  class Thermometer
    {
        ...
        public async void Connect()
        {
            ...
            foreach (var dev in MainPage.TestresultModel.BLEDevices)
            {
               if( dev.DeviceType == DeviceTypesenums.THERMOMETER  && dev.IsPaired)
               ...
               else if(dev.DeviceType == DeviceTypesenums.THERMOMETER && !dev.IsPaired)
                {
                   ...
                   string.Format(CommunicationCommands.THERMOnotpaired, "Device is not paried."));
            ...
            {                
                MainPage.mainPage.commChannel.SendMessageToMCC(
                   string.Format(CommunicationCommands.THERMORError, "Device not found."));
            ...
            {
                MainPage.mainPage.commChannel.SendMessageToMCC(
                     string.Format(CommunicationCommands.THERMORError, "Device Unreachable."));
            ...
            else
            {
                MainPage.TestPanelVM.InstuctionNoteCallBackCompleted?.Invoke(DeviceTypesenums.THERMOMETER, "");
                MainPage.mainPage.commChannel.SendMessageToMCC(
                     string.Format(CommunicationCommands.THERMORError, "Failed to connect. " + result.Status.ToString()));
        ...
        async void SelectService(BLEDeviceInfo dev)
        {
            try
            {
                ...
                if (accessStatus == DeviceAccessStatus.Allowed)
                {
                    ...
                    {
                        MainPage.mainPage.commChannel.SendMessageToMCC(
                     string.Format(CommunicationCommands.THERMORError, "Error accessing service " + result.Status.ToString()));
                ...
                {
                    MainPage.mainPage.commChannel.SendMessageToMCC(
                    string.Format(CommunicationCommands.THERMORError, "Error accessing service " + accessStatus.ToString()));
            ...
            {
                MainPage.mainPage.commChannel.SendMessageToMCC(
                    string.Format(CommunicationCommands.THERMORError, "Exception " + ex.Message));
            ...
            {
                MainPage.mainPage.commChannel.SendMessageToMCC(
                    string.Format(CommunicationCommands.THERMORError, "Thermometer Characteristic not found."));
            ...
            {
                MainPage.mainPage.commChannel.SendMessageToMCC(
                   string.Format(CommunicationCommands.THERMORError, "Descriptor read failure: " + resultch.Status.ToString()));
       ...
       async void SubscribeDeviceReading()
        {
            ...
            {
                MainPage.mainPage.commChannel.SendMessageToMCC(
                 string.Format(CommunicationCommands.THERMORError, $"Error registering for value changes: {status}"));
